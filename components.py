...
def display_app_title():
    ...
    タイトル表示
    """
    st.markdown(f"<div style='text-align:center;'><h2>{ct.APP_NAME}</h2></div>", unsafe_allow_html=True)
...
def display_select_mode():
    ...
    """
    with st.sidebar:
        st.markdown("<b>利用目的</b>", unsafe_allow_html=True)
        ...
        st.divider()
        st.markdown("<b>【社内文書検索】</b>", unsafe_allow_html=True)
        st.markdown("入力内容と関連性が高い社内文書のありかを検索できます。")
        st.code("【入力例】\n社員の育成方針に関するMTGの議事録", wrap_lines=True, language=None)
        st.markdown("<b>【社内問い合わせ】</b>", unsafe_allow_html=True)
...
def display_initial_ai_message():
    ...
    with st.chat_message("assistant"):
        st.markdown("こんにちは。私は社内文書の情報をもとに回答する生成AIチャットボットです。左側のサイドバーで利用目的を選択し、画面下部のチャット欄からメッセージを送信してください。")
        st.warning("未入力または入力した内容が空の場合の応答を表示します。", icon=ct.WARNING_ICON)
